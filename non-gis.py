...
class ExtractorConfig:
    """Konfigurasi sentral untuk ekstraksi modul non-spasial (mengabaikan GIS)."""
    
    # Target Direktori Default
    DEFAULT_TARGET = r"C:\Users\PC\Documents\Dev\E-LIMBAD\limbah-fe"
    DEFAULT_OUTPUT = r"C:\Users\PC\Documents\Dev\E-LIMBAD\limbah-fe\limbah-fe-non-gis.txt"

    # Folder yang diblokir murni (Blacklist Folder)
    ...
    # Berkas yang disingkirkan agar konteks LLM ringan
    ...
    # A. Kata kunci GIS pada nama berkas atau jalur folder untuk dieksklusi
    PATH_KEYWORDS: List[str] = [
        "gis", "map", "spatial", "layer", "aqi", "wind", 
        "leaflet", "geo", "telemetry", "location"
    ]

    # B. Kata kunci GIS di dalam kode sumber untuk dieksklusi
...
class LLMContextOptimizer:
    """Utility untuk memampatkan teks agar menghemat token LLM."""
...
class NonGisCodebaseExtractor:
    """Controller utama untuk manajemen pemindaian kode Non-GIS."""
    ...
    def _should_process(self, relative_path: Path, file_path: Path) -> bool:
        ...
        # 2. Validasi Root Files Utama (package.json, tsconfig.json, dsb tetap dibawa untuk konteks dependency)
        if is_root_file:
        ...
        # 5. Sensor Eksklusi Spasial/GIS (Logika Terbalik)
        ...
        if any(kw in lower_path for kw in self.config.PATH_KEYWORDS):
            return False  # LANGSUNG DIABAIKAN jika mengandung keyword GIS di nama/jalur file

        # Jika jalur aman, lakukan Deep Scan pada kode sumber
        try:
            content = file_path.read_text("utf-8", errors="ignore")
            if any(kw in content for kw in self.config.CONTENT_KEYWORDS):
                return False  # LANGSUNG DIABAIKAN jika kode menggunakan library/konsep GIS
            
            # OPTIMASI: Jika file terbukti non-GIS dan lolos sensor, 
            # simpan ke cache agar tidak perlu membaca disk dua kali saat penulisan
    ...
    def execute(self):
        ...
        logger.info(f"🔍 Mulai memindai file NON-GIS di: {self.target_path}")
    ...
    def _write_output(self, files: List[Path], original_size: int):
        ...
        with open(self.output_file, "w", encoding="utf-8") as f:
            f.write("=== KUMPULAN KODE NON-SPASIAL (NON-GIS MODE - COMPRESSED) ===\n\n")
            
            for file_path in files:
                relative_path = file_path.relative_to(self.target_path)
                try:
                    # Ambil data dari RAM Cache jika ada (seperti file src), jika tidak (seperti file root), baru baca disk
                    ...
                    logger.info(f"-> Menyalin Modul Non-GIS: {relative_path}")
                except Exception as e:
                    logger.error(f"-> Gagal membaca {relative_path}: {e}")

        logger.info(f"\n✅ Selesai! Ekstraksi modul Non-GIS berhasil disimpan pada:\n   {self.output_file}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LLM Non-GIS Codebase Extractor - Vite/React SPA")
    ...
    extractor = NonGisCodebaseExtractor(target_dir=args.dir, output_file=args.out)
    extractor.execute()