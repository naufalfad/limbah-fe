...
export default function SuperAdminPage() {
  ...
  // Mengambil state dan actions dari Zustand Store (API Connected)
  const {
    currentUser,
    users,
    auditLogs,
  ...
  // State untuk API Loading
  const [isDataLoading, setIsDataLoading] = useState(true);

  // Local UI States (Untuk simulasi Gateway & Peta)
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    // Panggil action update role di Zustand (Nembak API)
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <Card className="rounded-none border border-slate-200 shadow-sm bg-white flex flex-col">
                ...
                <div className="flex-1 flex flex-col divide-y divide-slate-100 font-sans text-xs">
                  {auditLogs.length === 0 ? (
                    <div className="p-8 text-center text-slate-400 font-bold">Belum ada riwayat aktivitas.</div>
                  ) : (
                      ...
                      <TableRow key={u.id} className="hover:bg-slate-50 transition-colors border-b last:border-b-0 h-12">
                        ...
                        <TableCell>
                          <select
                            ...
                            className="h-8 text-[11px] border border-slate-200 rounded-none px-2 bg-white font-bold text-slate-600 focus:outline-none focus:border-emerald-500"
                          >
...
function StatCard({ label, value, icon, color }: any) {
  ...
  return (
    <div className="border border-slate-200 p-4 shadow-sm flex items-start justify-between bg-white rounded-none">
